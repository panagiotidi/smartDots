...
from data.utils import create_name, is_label_ok
...
def is_local(row):
    if os.path.exists(original_path + row['Species'] + '_' + str(row['ModalAge_AllReaders']) + '_' + row['ImageName']):
        return True
    else:
        return False
...
def process_and_save(dataset, set_path):
    ...
    for index, row in dataset.iterrows():
        try:
            ...
            dataset = dataset[dataset['ImageName'] != img_base_name]
            traceback.print_exc()
...
if __name__ == '__main__':
    ...
    train, val = train_test_split(data, test_size=VAL_SPLIT)
